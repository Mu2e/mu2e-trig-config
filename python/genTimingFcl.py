...
def parse_sequences(verbose):
    with open('mu2e-trig-config/core/trigSequences.fcl', 'r') as f:
        ...
        for line in f:
            ...
            if ended and verbose > 1: print(f'    {current} path is {paths[current]}')
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--name",
                        dest="name", default="trigger_timing",
    ...
    if not name.endswith('.fcl'): name = name + '.fcl'

    if verbose > 0: print(f'Using output fcl {name} with trigger paths {paths}')
